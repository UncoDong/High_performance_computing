...
subroutine jacobi2d_sub(n, tol, maxiter, iter, dumax, nthreads)
	...
	do i = 1,2*n-1
		if(i<=n)then !遍历上三角矩阵
			...
			do j = n-i+1,n
			uold(j,j+i-n) = u(j,j+i-n)
			!print *,j,j+i-n
			u(j,j+i-n) = u(j,j+i-n)*(1-w)+w*0.25d0*(u(j-1,j+i-n) + u(j+1,j+i-n) + &
                                 u(j,j+i-n-1) + u(j,j+i-n+1) + h**2*f(j,j+i-n))
                	dumax = max(dumax, abs(u(j,j+i-n)-uold(j,j+i-n)))
			enddo
		else !遍历下三角矩阵
			!$omp parallel do reduction(max : dumax)
			do j = 1,2*n-i
			uold(j,i-n+j) = u(j,i-n+j)
			!print *,j,i-n+j
			u(j,i-n+j) = u(j,i-n+j)*(1-w)+w*0.25d0*(u(j-1,i-n+j) + u(j+1,i-n+j) + &
                                 u(j,i-n+j-1) + u(j,i-n+j+1) + h**2*f(j,i-n+j))
                	dumax = max(dumax, abs(u(j,i-n+j)-uold(j,i-n+j)))
			enddo
		endif
	enddo
